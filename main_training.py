...
import graph_feature_extraction  # custom.py
...
add_hydrogen = CONFIG.add_hydrogen
...
file_name = f'graph_feature_len_{filtered_num}_num_{random_pick_num}_scaled_True_ECFP_True_desc_True_H_False_3D_False_graph.h5'
...
if os.path.exists(graph_raw_path):
    ...
    file_raw['graph'] = [graph_feature_extraction.generate_mol_graph(mol=Chem.MolFromSmiles(i),
                                                                     add_hydrogen=add_hydrogen,
                                                                     use_pos=use_pos) for i in
                         tqdm(file_raw['smiles'], total=len(file_raw))]
    ...
    file_raw.to_hdf(graph_raw_file_path, key='graph', mode='w')
    print(f"Graph object is saved at '{graph_raw_file_path}'")
    graph_raw = file_raw.copy()
...
for i, target_name in tqdm(enumerate(Y_total_list), total=len(Y_total_list)):
    ...
    for epoch in range(0, epochs):
        ...
        if epoch % 1 == 0:
            print(f"Target: {target_name} | Epoch {epoch} | Train Loss (MSE): {train_loss:.4f} | Val Loss (MSE): {val_loss:.4f}")
